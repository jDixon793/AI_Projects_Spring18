...
handsign rpsAgentPHDinCatBehavior(const vector<handsign> focal, const vector<handsign> opponent)
{
  ...
  if(opponent.size() != 0)
  {
	...
	switch(hold)
	{
		...
		case 2:
			if(meS >= meP && meS>=meR){return rock;}
			if(meR >  meP && meR>meS){return paper;}
			if(meP > meS && meP> meR){return scissors;}
		break;
	 
		case 3:
			if(oppS >= oppP && oppS>=oppR){return rock;}
			if(oppR > oppP && oppR>oppS){return paper;}
			if(oppP > oppS && oppP> oppR){return scissors;}
		break;
		
		case 4://decay frequency http://www.rpscontest.com/entry/13015
			if(number < exp(rateR)){return rock;}
			else if(number < exp(rateR) + exp(rateP)){return paper;}
