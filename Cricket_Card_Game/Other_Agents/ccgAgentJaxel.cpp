...
int ccgAgentJaxel(Hand hand, Card lastBowledCard, bool isBatting, const MatchState &match)
{
   ...
   else // is bowling
   {
      ...
      // maybe we can make it play a lot of the same suit to try to force a wicket down
      
      int whichCard;
      Card card;
      int highestCardIndex=-1;
      //Suit lastBolwedCard.getSuit();
       for (whichCard = 0; whichCard < numCardsPerHand; whichCard += 1)
      {
         card = hand.getCard(whichCard);
