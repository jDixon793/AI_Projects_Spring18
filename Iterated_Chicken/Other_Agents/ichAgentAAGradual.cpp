...
strategy ichAgentAAGradual(const vector<strategy> focal, const vector<strategy> opponent)
{
  ...
  else                                             //Play the strat 
  {
     ...
     else if(focal.size()>2&&focal.at(opponent.size()-1)==playC&&focal.at(opponent.size()-2)==playD)
        return playC; 
     else if (opponent.at(opponent.size()-1)==playD) //if the opponent played a D against us we must retaliate strong
     {
...
/*
 ...
 This Agent wants to play C the whole game unless the opponent plays D
      ...
      this should do a good job of encouraging cooperation between Agents 
*/
