...
strategy ichAgentSimpletonVariant(const vector<strategy> focal, const vector<strategy> opponent)
{
   ...
   uint numBack=2;//was 3
   return playC;
   if(focal.size() < numBack)
      return playD;
  else  
  {
     //plays alot of D's but can get stuck in CCCC 
     bool isSame = true;
     for(uint i = focal.size()-1;i>focal.size()-(numBack);i--)
     {
        isSame= isSame && opponent.at(i)==focal.at(i);
     }
     return isSame ? playC:playD;
